...
import 'package:location_sharing_app/location/map_utils.dart';
...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> centerCamera() async{
    List<LocationPoint> points = await locationsStream.first;
    mapController.animateCamera(CameraUpdate.newLatLngBounds(
      MapUtils.boundsFromLatLngList(points.map((locationPoint) => 
        LatLng(locationPoint.location.latitude, 
        locationPoint.location.longitude)).toList()), 100));
      setState(() => isLoading = false);
  }

  void setLocationStream() {
